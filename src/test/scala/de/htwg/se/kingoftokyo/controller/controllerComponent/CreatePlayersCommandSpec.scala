...
class CreatePlayersCommandSpec extends WordSpec with Matchers {
  ...
  "CreatePlayersCommand" when {
    "new" should {
      val createPlayersCommand = new CreatePlayersCommand("Simon,Alex", controller)

      "undo" in {
        createPlayersCommand.undoStep
        controller.state should equal (controllerEmpty.state)
      }

      "redo" in {
        createPlayersCommand.redoStep
        controller.state should equal (State.WaitFor1stThrow)
      }
    }
  }
}
