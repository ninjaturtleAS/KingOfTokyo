...
class PlayersSpec extends WordSpec with Matchers {
  ...
  "Players at start" when {
    "new" should {
      ...
      val testPlayers = Vector(Player("Alex", 5, 0, 10), Player("Simon", 5, 0, 10), Player("Klaus", 5, 5, 10), Player("Marco", 5, 0, 10))
      val plTest = Players(testPlayers)
      ...
      "cut KoT" in {
        val tupel = plTest.cutKOT(testPlayers, 1, 3)
        tupel._1.length should be(3)
        tupel._2 should be(2)
        tupel._3 should be(2)
        tupel._4 should be(true)

        val tupel2 = plTest.cutKOT(testPlayers, 1, 0)
        tupel2._1.length should be(3)
        tupel2._2 should be(0)
        tupel2._3 should be(0)
        tupel2._4 should be(true)
      }
      val cutTestPlayers = Vector(Player("Alex", 5, 1, 10), Player("Simon", 5, 0, 10), Player("Klaus", 5, 5, 10), Player("Marco", 5, 0, 10))
      "cutPlayers" in {
        val tupel = plTest.cutPlayers(cutTestPlayers, 0)
        tupel._1 should be(Vector(Player("Alex", 5, 1, 10), Player("Klaus", 5, 5, 10)))
