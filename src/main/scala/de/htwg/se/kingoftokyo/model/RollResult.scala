...
case class RollResult(result: Vector[Int]) {
  ...
  def mapFacesToString(i: Int): String = {
    ...
    override def toString(): String = {
      mapFacesToString(result(0))
        .concat(mapFacesToString(result(1)))
        .concat(mapFacesToString(result(2)))
        .concat(mapFacesToString(result(3)))
        .concat(mapFacesToString(result(4)))
        .concat(mapFacesToString(result(5)))
  }
